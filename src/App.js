...
class App extends React.Component {
  ...
  processBoard() {
    ...
    if (!this.state.boardState.includes(2) && !won) {
      ...
      this.setState({ active: false });
    } else if (this.state.mode == "CPU" && this.state.turn == 1 && !won) {
      this.makeCPUMove();
    }
  }
  makeCPUMove() {
      ...
      patterns.forEach((pattern) => {
        if (pattern.includes(index)) {
          ...
          pattern.forEach((p) => {
            ...
            score += p == index ? 0 : CPUScore[this.state.boardState[p]];
          });
      ...
      (previousState) => {
        return {
          boardState: previousState.boardState
            .slice(0, id)
            .concat(previousState.turn)
            .concat(previousState.boardState.slice(id + 1)),
          turn: (previousState.turn + 1) % 2,
        };
      },
      () => {
        this.processBoard();
  ...
  handleModeChange(e) {
    e.preventDefault();
    if (e.target.getAttribute("href").includes("CPU")) {
      e.target.style.background = "#d4edda";
      document.querySelector("#twop").style.background = "none";
      this.setState({ mode: "CPU" });
    ...
    return (
      <div>
        <div class="container jumbotron" id="container">
          ...
          <p>
            <a href="./?CPU" onClick={this.handleModeChange} id="CPU">
              Battle CPU
            </a>{" "}
            ||
